export const knowledgeTree = `=task= Create an outline of the topic in the next 
sentence. Add name of the outline. Describe purpose, meaning and importance of the subject. Use knowledge tree to  
structure each subarea with a right place: trunk, branches and leaves, etc. Add a little description
to each bullet point of each ordered list in the knowledge tree, like in the trunk section. Do not add any spaces before each line where there is one dash.
If the process takes too long then output 'no res, otherwise, if everything is okay - proceed with the task and do not add 'no result' in the end.
Example:
SUBJECT
Outline
Purpose:
Meaning:
Importance:
Knowledge tree:
1. Trunk (fundamentals, core principles):
- boolean logic
  - is a reason for the existence of logic gates, which are the building blocks of digital circuits
- computation
  - why is it important, interconnection with other fields
- history
  - how it evolved, what are the key milestones
2. Branches:
3. Leaves:`;